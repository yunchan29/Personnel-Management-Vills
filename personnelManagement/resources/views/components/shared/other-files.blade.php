@props(['otherFiles' => collect(), 'deleteRoute' => 'employee.files.destroy'])
...
<div x-show="activeTab === 'files'" x-cloak>
            ...
            <div class="max-h-60 overflow-y-auto border border-gray-200 rounded-md shadow-sm">
                <ul class="divide-y divide-gray-200 text-sm text-gray-800">
                    @foreach($otherFiles as $file)
                        ...
                        <button
                            type="button"
                            onclick="confirmDelete({{ $file->id }}, '{{ $deleteRoute }}')"
        ...
        <div class="mb-6 border-b pb-4">
            ...
            <select :name="'additional_documents['+index+'][type]'"
                ...
                <template x-for="option in documentTypes" :key="option">
                    <option :value="option"
                            :disabled="documents.some((d, i) => i !== index && d.type === option)">
    ...
    <div class="text-center">
        <button type="button"
                @click="addDocument()"
                class="mt-2 bg-[#BD6F22] hover:bg-[#a75f1c] text-white font-semibold px-4 py-2 rounded-md transition">
            Add New
        </button>
    </div>
</div>
