<div x-data="applicantsHandler()" x-init="init()" class="relative">
            ...
            <tbody>
                @forelse($applications as $application)
                    <tr
                        ...
                        x-show="(showAll || '{{ $application->interview_date }}' === '') && !removedApplicants.includes({{ $application->id }})"
                        ...
                        <td class="py-3 px-4">
                            @if($application->user->active_status === 'Active' && $application->resume_snapshot)
                                <button @click="openResume('{{ asset('storage/' . $application->resume_snapshot) }}')"
                                    class="bg-[#BD6F22] text-white text-sm font-medium h-8 px-3 rounded shadow hover:bg-[#a95e1d]">View</button>
                        ...
                        <td class="py-3 px-4">
                            @if($application->user->active_status === 'Active')
                                <button @click="openProfile({{ $application->id }})"
                                    class="border border-[#BD6F22] text-[#BD6F22] text-sm font-medium h-8 px-3 rounded hover:bg-[#BD6F22] hover:text-white">View</button>
                            @else
                                <span class="text-gray-400 italic">Inactive</span>
                            @endif
                        </td>
    ...
    <template x-if="!['interviewed','declined','for_interview'].includes(applicants.find(a => a.id === {{ $application->id }})?.status || '{{ $application->status }}')">
        <span class="text-xs bg-gray-200 text-gray-800 px-2 py-1 rounded-full transition-colors duration-300">Pending</span>
                            ...
                            @if($application->user->active_status === 'Active')
                                <div class="flex gap-2">
                                    ...
                                    <button
                                        ...
                                        class="bg-green-600 text-white text-sm font-medium h-8 px-3 rounded hover:bg-green-700 disabled:opacity-50"
                                        :disabled="['interviewed', 'declined'].includes(applicants.find(a => a.id === {{ $application->id }})?.status)">Manage</button>
                                </div>
                            @else
                                <span class="text-gray-400 italic">Inactive</span>
                            @endif
                ...
                @empty
                    <tr><td colspan="8" class="py-6 text-center text-gray-500">No applicants yet.</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
    ...
    <!-- ✅ Feedback Toast -->
    <div x-show="feedbackVisible" x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0 translate-y-4" x-transition:enter-end="opacity-100 translate-y-0" x-transition:leave="transition ease-in duration-200" x-transition:leave-start="opacity-100 translate-y-0" x-transition:leave-end="opacity-0 translate-y-4" class="fixed bottom-6 right-6 bg-green-600 text-white px-5 py-4 rounded-xl shadow-lg z-50 w-80 overflow-hidden" x-cloak>
